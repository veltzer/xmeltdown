...
/*	int		flag1 = 1;
	...
	for (bone = 0; bone < 11; bone++) {
		fromray =	(RayPtr) ((int) &(fromskel[which]->torso) + 
						(int) (bone * bonesize));
		toray = 	(RayPtr) ((int) &(toskel[which]->torso) + 
						(int) (bone * bonesize));
		tweenray =	(RayPtr) ((int) &(tweenskel[which]->torso) +
						(int) (bone * bonesize));
...
*/
		...
		tweenray->t =(fromray->t * factor +
				toray->t * (1.0 - factor));
		tweenray->r =(fromray->r * factor + 
				toray->r * (1.0 - factor));

	}

	tweenskel[which]->headradius = fromskel[which]->headradius * factor +
			toskel[which]->headradius * (1.0 - factor);
	tweenskel[which]->waistx = fromskel[which]->waistx * factor +
			toskel[which]->waistx * (1.0 - factor);
	tweenskel[which]->waisty = fromskel[which]->waisty * factor +
			toskel[which]->waisty * (1.0 - factor);
...
char	*filename;
{
	...
	while (okayflag >= 0) {
		...
		if (okayflag >= 0) {
			...
			retval[numread-1] = (Skeleton *)
				calloc (1, sizeof (Skeleton));
