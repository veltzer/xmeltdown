...
*/
	if (ButtonDown == 1) {
		...
		for (		totaltheta = 0, fooray = targetray->anchor; 
				fooray;
				fooray = fooray->anchor) {
			totaltheta += fooray->t;
		}
...
*/
	for (	stepray = &(bones.torso); 
		stepray <= &(bones.ltricep); 
		stepray = (RayPtr) ((int) stepray + (int) stepsize)) {
